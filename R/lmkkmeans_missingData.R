...
#' Defaults to FALSE.
#' @param variableUpperBounds Vector of variable upper bounds (1 corresponds to available data points,
#' 0 to missing data).
#' @param thetaMultiplier Matrix to be multiplied by theta at each iteration.
...
lmkkmeans_missingData <- function(Km, parameters, verbose = FALSE, variableUpperBounds, thetaMultiplier){
  state <- list()
  state$time <- system.time({
    ...
    for (iter in 1:parameters$iteration_count) {
      ...
      problem$bx <- rbind(blx = rep(0, N * P), bux = rep(1, N * P)) #variableUpperBounds
      ...
      Theta <- matrix(result$sol$itr$xx, N, P, byrow = FALSE)*thetaMultiplier
